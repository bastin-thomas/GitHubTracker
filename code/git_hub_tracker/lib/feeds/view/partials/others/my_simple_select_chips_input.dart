...
class MySelectChipsInput extends StatefulWidget {
  ...
  final List<int> selectedChipsIndex;

  @override
  State<MySelectChipsInput> createState() => _SelectChipsInputState();
}

class _SelectChipsInputState extends State<MySelectChipsInput> {
  ...
  void initState() {
    ...
    if (widget.preSelectedChips != null) {
      widget.selectedChipsIndex.addAll(widget.preSelectedChips!);
    }
  }
    ...
    for (int i = 0; i < widget.chipsText.length; i++) {
      chips.add(GestureDetector(
        onTap: () {
          ...
          if (!widget.onlyOneChipSelectable!) {
            setState(() {
              if (widget.selectedChipsIndex.contains(i)) {
                widget.selectedChipsIndex.remove(i);
              } else {
                widget.selectedChipsIndex.add(i);
              }
            });
            if (widget.onTap != null) {
              String output = '';
              for (int i in widget.selectedChipsIndex) {
          ...
          // only one chip can be selected at a time
            ...
            setState(() {
              widget.selectedChipsIndex.clear();
              widget.selectedChipsIndex.add(i);
            });
            if (widget.onTap != null) {
        ...
        child: Container(
          ...
          decoration: widget.selectedChipsIndex.contains(i)
          ...
          child: Row(
            ...
            children: [
              if (widget.selectedPrefixIcon != null &&
                  widget.selectedChipsIndex.contains(i))
              ...
              Flexible(
                child: Text(
                  widget.chipsText[i],
                  style: widget.selectedChipsIndex.contains(i)
